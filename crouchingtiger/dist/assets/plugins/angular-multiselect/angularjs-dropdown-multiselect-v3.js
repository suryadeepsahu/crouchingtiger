(function(angular) {
    ...
    var directiveModule = angular.module('angularjs-dropdown-multiselect-v3', []);
    ...
    directiveModule.directive('ngDropdownMultiselectV3', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
        return {
            restrict: 'AE',
            scope: {
                selectedModel: '=',
            ...
            template: function(element, attrs) {
                ...
                if (checkboxes) {
                    template += '<div class="checkbox"><label><input class="checkboxInput" type="checkbox"  ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" ng-disabled="option.disabled" " /> <span dm-dropdown-static-include="{{settings.template}}"></div></label></span></a>';
                } else {
                ...
                $scope.toggleDropdown = function() {
                    ...
                    if ($scope.settings.enableSearch) {
                        ...
                        if($scope.selectedModel.length !== 0){
                            $scope.options.forEach(function(op){
                            op.disabled = false;
                         })
                        for (var i = $scope.selectedModel.length - 1; i >= 0; i--) {
                           disablingOption($scope.selectedModel[i].id);
                                        }
                    } else{
                         $scope.options.forEach(function(op){
                            op.disabled = false;
            ...
            var disabledOptions ={
                    "AllDays" : ["Mon-Fri","Sat-Sun","Mon","Tue","Wed","Thu","Fri","Sat","Sun"],
            ...
            var convertToAll = [];
            var disablingOption =function(op){
                var isPresent = 0;
                for (var i = $scope.selectedModel.length - 1; i >= 0; i--) {
                   if( $scope.selectedModel[i].id == op)
                                        {
                        isPresent++;
                                        }
                                    }
                if(isPresent)  {
                    var disabledValue = disabledOptions[op];
                    disabledValue.forEach(function(op){
                        convertToAll = [];
                        for (var a = $scope.selectedModel.length - 1; a >= 0; a--) {
                            if($scope.selectedModel[a].id == op){
                                $scope.selectedModel.splice(a, 1)
                              }
                            convertToAll.push($scope.selectedModel[a].id)
                            for (var i = 0; i < $scope.options.length; i++) {
                               if($scope.options[i].id === op){
                                ...
                                $scope.selectedModel.forEach(function(option){
                                    disablingOption(option.id);
                                })
                              }
                        }       
             var setSelectedValues=function(seleced){
                    var selecedOptions = [
                    ...
                    if($scope.selectedModel.length > 0 ){
                        ...
                        disablingOption(seleced)
                       /* for (var a = selecedOptions.length - 1; a >= 0; a--) {
                            var matchedArray = convertToAll.filter(element => selecedOptions[a].includes(element));   
                            console.log(matchedArray);
                            if(matchedArray.length === selecedOptions[a].length) {
                                var finalObj = null;
                                clearObject($scope.selectedModel);
                                $scope.selectedModel.splice($scope.selectedModel.length)
                                $scope.selectedModel = [];
                                console.log("$scope.selectedModel==",$scope.selectedModel)
                                $scope.selectedModel.push({
                                    'id':"AllDays"
                                });
                                disablingOption("AllDays");
                                break;
                            }  
                        }*/
                    }else{
                ...
                $scope.externalEvents = {
                    ...
                    onItemCustomSelect: angular.noop, 
                        ...
                        if ($scope.settings.smartButtonMaxItems > 0) {
                            ...
                            angular.forEach($scope.options, function(optionItem) {
                                if ($scope.isChecked($scope.getPropertyForObject(optionItem, $scope.settings.idProp))) {
                                    var displayText = $scope.getPropertyForObject(optionItem, $scope.settings.idProp);//$scope.settings.displayProp
                ...
                $scope.setSelectedItem = function(id, dontRemove, fireSelectionChange) {    
                    ...
                    if ($scope.singleSelection) {
                        ...
                        if (fireSelectionChange) {
                            ...
                            $scope.externalEvents.onItemCustomSelect($scope.schIndex);
                        }

...
angular.module('turn/stickyTableHeader3', ['watchDom']).value('stickyTableHeaderOptions3', {
  cloneClassName: 'sticky-clone3',
  stuckClassName: 'sticky-stuck3',
  interval: 10,
  observeHeaderInterval: 100
}).service('stickyTableHeaderUtil3', function () {
  this.guard = function (fn, condition) {
    return function () {
      return condition() ? fn.apply(this, arguments) : false;
    };
  };
}).directive('stickyTableHeader3', [
  '$timeout',
  '$window',
  'stickyTableHeaderOptions3',
  'stickyTableHeaderUtil3',
  'watchDom',
  function ($timeout, $window, stickyTableHeaderOptions3, stickyTableHeaderUtil3, watchDom) {
    var options = stickyTableHeaderOptions3, util = stickyTableHeaderUtil3;
    return {
      restrict: 'A',
      scope: {
        disabled: '=',
      ...
      link: function (scope, element) {
        angular.extend(scope, {
          ...
          checkScroll: ifClone(function () {
            ...
            if (!scope.stuck && scrollY >= scope.offset.top + 30) {
              scope.setClonedCellWidths();
              scope.setStuck(true);

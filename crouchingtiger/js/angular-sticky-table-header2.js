...
angular.module('turn/stickyTableHeader2', ['watchDom']).value('stickyTableHeaderOptions2', {
  cloneClassName: 'sticky-clone2',
  stuckClassName: 'sticky-stuck2',
  interval: 10,
  observeHeaderInterval: 100
}).service('stickyTableHeaderUtil2', function () {
  this.guard = function (fn, condition) {
    return function () {
      return condition() ? fn.apply(this, arguments) : false;
    };
  };
}).directive('stickyTableHeader2', [
  '$timeout',
  '$window',
  'stickyTableHeaderOptions2',
  'stickyTableHeaderUtil2',
  'watchDom',
  function ($timeout, $window, stickyTableHeaderOptions2, stickyTableHeaderUtil2, watchDom) {
    var options = stickyTableHeaderOptions2, util = stickyTableHeaderUtil2;
    return {
      restrict: 'A',
      scope: {
        disabled: '=',
      ...
      template: '<div class="transclude-header2" ng-transclude></div>',
      transclude: true,
      link: function (scope, element) {
        angular.extend(scope, {
          ...
          createClone: function () {
            // added by Trevor Casey on 6.15.16 as a patch fix to minimize bugs on windows
            if (navigator.platform === "Win32" && navigator.userAgent.includes("Chrome") ) {
                console.log('removed sticky header of sub-groups table due to potential bug');
            } else {

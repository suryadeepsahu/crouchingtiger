	...
	for (var i = 0; i < charts.length; i++) {
		...
		if (charts[i].type === "bar") {
			...
			charts[i].chart = dc.barChart("#chart" + i)
				...
				//;
				//.barPadding(.5)
				//.x(d3.scale.linear().domain([new Date(minMonth), d3.time.hour.offset(new Date(maxMonth), 24)]))
		}
...
function chartSettings(k) {
	...
	$('#chart-settings [name=dimensiontype]').val(charts[k].dimensiontype);
	$('#chart-settings [name=interval]').val(charts[k].interval);
	$('#chart-settings [name=group]'    ).val(charts[k].group);
	$('#chart-settings [name=grouptype]').val(charts[k].grouptype);
...
function chartSettingsSave() {
	...
	charts[id].dimensiontype = $('#chart-settings [name=dimensiontype').val();
	charts[id].interval		= $('#chart-settings [name=interval').val();
	charts[id].group		= $('#chart-settings [name=group]'    ).val();
	charts[id].grouptype	= $('#chart-settings [name=grouptype]').val();
...
function saveChart(index) {
	if (charts[index].deleted) {
		...
		var data = {
			...
			dimensiontype: charts[index].dimensiontype,
			interval:  charts[index].interval,
			group:     charts[index].group,
			grouptype: charts[index].grouptype,

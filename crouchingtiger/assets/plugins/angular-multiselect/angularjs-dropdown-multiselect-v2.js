(function(angular) {
    ...
    var directiveModule = angular.module('angularjs-dropdown-multiselect-v2', []);
    ...
    directiveModule.directive('ngDropdownMultiselectV2', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
        return {
            restrict: 'AE',
            scope: {
                selectedModel: '=',
            ...
            template: function(element, attrs) {
                ...
                ;
                var template = '<div id="customized" class="multiselect-parent btn-group dropdown-multiselect" ng-class="{open: open}">';
                template += '<button ng-disabled="disabled" type="button" class="dropdown-toggle" ng-class="settings.buttonClasses" ng-click="toggleDropdown()" style="display:none" >{{getButtonText()}}&nbsp;<span class="caret"></span></button>';
                ...
                template += '<ul class="dropdown-menu dropdown-menu-form" ng-if="open" ng-style="{display: open ? \'block\' : \'block\', height : settings.scrollable ? settings.scrollableHeight : \'auto\', overflow: \'auto\' }" >';
                ...
                template += '<li ng-if="settings.enableSearch"><div class="dropdown-header" style="padding:3px 10px"><input type="text" style="padding:7px 3px" class="form-control searchField" ng-keydown="keyDownSearchDefault($event); keyDownSearch($event, input.searchFilter);" ng-style="{width: \'100%\'}" ng-model="input.searchFilter" placeholder="{{texts.searchPlaceholder}}"  ng-change="getExactMatch(input.searchFilter, options)" ng-model="input.searchFilter"></li>';
                template += '<li ng-if="settings.enableSearch" class="divider"></li>';
                template += '<li ng-if="searchedResult.length == 0 || exactmatch == false"><a ng-click="addSearchFilter(input.searchFilter)" style="color:orange">Add custom sources "{{input.searchFilter}}"</a></li>';
                if (groups) {
                    ...
                    template += '<li ng-class="{\'active\': isChecked(getPropertyForObject(option,settings.idProp)) && settings.styleActive}" ng-repeat-end role="presentation"></li>';
                } else {
                    template += '<li ng-class="{\'active\': isChecked(getPropertyForObject(option,settings.idProp)) && settings.styleActive}" role="presentation" ng-repeat="option in options | filter:getFilter(input.searchFilter, options) as searchedResult" ng-disabled="true" >';
                }
                ...
                if (checkboxes) {
                    template += '<div class="checkbox"><label><input class="checkboxInput" type="checkbox" ng-disabled="option.disabled" ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" /> <span dm-dropdown-static-include="{{settings.template}}"></div></label></span></a>';
                } else {
            ...
            //  $scope.toggleDropdown();
                ...
                $scope.settings = {
                    ...
                    closeOnBlur: true,
                    displayProp: 'custom_source_name',
                    idProp: 'custom_source_id',
                ...
                $scope.texts = {
                    ...
                    selectionOf: '/',
                    searchPlaceholder: 'Add a New Source',
                ...
                $scope.close = function() {
                    if(!$scope.settings.openByDefault)
                        $scope.open = false;
                    ...
                    if ($scope.orderedItems) {
                        $scope.orderedItems.forEach(function(item) {
                            if (item[$scope.groupBy] === currentGroup) {
                                //console.log("selectCurrentGroup..called")
                ...
                $scope.addSearchFilter = function(term){
                    $scope.externalEvents.onAddItem(term);
                ...
                $scope.getPropertyForObject = function(object, property) {
                    //console.log("getPropertyForObject called")
                    if (angular.isDefined(object) && object.hasOwnProperty(property)) {
                        //  console.log("getPropertyForObject return=",object[property])
                ...
                //  console.log("keyDownSearchDefault($event)",event.target.parentNode.parentNode)
                ...
                $scope.keyDownSearch = function(event, searchFilter) {
                    ...
                    if (event.keyCode === 13) {
                        if ($scope.settings.selectionLimit === 1 && $scope.settings.enableSearch) {
                            ...
                            //console.log("Enable to search....")
                            $scope.selectAll();
                        }
                    }
                };

                $scope.getExactMatch = function(searchFilter, options) {
                    $scope.exactmatch =  true;

                    if(searchFilter == '' && options != undefined){//page load condtion don't want to show
                        $scope.exactmatch =  true;
                    }else if(searchFilter != "" && options == undefined){//while adding records for 1st time
                            $scope.exactmatch =  false;
                        }
                    if(options != undefined && searchFilter != ""){//when already existing records are searched
                        if(options.length > 0){
                            for(var option in options){
                                if(options[option].custom_source_name.toString() === searchFilter.toString()){
                                    $scope.exactmatch =  true; break;
                                }else{
                                    $scope.exactmatch =  false;
                                }
                            }
        ...
        //console.log("keyDownToggleSearch..")
                    if (!$scope.settings.keyboardControls) {
                        return;
                    }
                    if (event.keyCode === 13) {
    ...
    function contains(collection, target) {
        ...
        //console.log("containsTarget",containsTarget)

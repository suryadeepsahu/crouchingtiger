(function(angular) {
    ...
    var directiveModule = angular.module('angularjs-dropdown-multiselect-v4', []);
    ...
    directiveModule.directive('ngDropdownMultiselectV4', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
        return {
            restrict: 'AE',
            scope: {
                selectedModel: '=',
            ...
            template: function(element, attrs) {
                ...
                if (checkboxes) {
                    template += '<div class="checkbox"><label><input class="checkboxInput" type="checkbox" ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" /> <span title="{{getPropertyForObject(option, settings.displayProp)}}" dm-dropdown-static-include="{{settings.template}}"></div></label></span></a>';
                } else {
                    template += '<span data-ng-class="{\'glyphicon glyphicon-ok\': isChecked(getPropertyForObject(option,settings.idProp))}"> </span> <span title="{{getPropertyForObject(option, settings.displayProp)}}" dm-dropdown-static-include="{{settings.template}}"></span></a>';
                }
            ...
            link: function($scope, $element, $attrs) {
                ...
                $scope.settings = {
                    ...
                    template: '{{getPropertyForObject(option, settings.displayProp) | truncate_campGroupName }}',
                    searchField: '$',
